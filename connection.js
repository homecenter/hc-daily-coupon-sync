...
FTP.prototype.connect = function (options, cb) {
  var self = this;
  ...
  if (this.options.socksproxy === undefined) {
    ...
    Socks.createConnection(options, function (err, soc, info) {
      if (err) cb(makeError(999, "SOCKS connect timed out"));
      else {
        socket = soc;
        socket.setTimeout(0);
        socket.setKeepAlive(true);
...
FTP.prototype.mkdir = function (path, recursive, cb) {
  ...
  if (!recursive) this._send("MKD " + path, cb);
    ...
    abs = path[0] === "/";
    if (path.indexOf("/") === -1) this._send("MKD " + path, cb);
    else {
...
FTP.prototype._pasvPostConnect = function (ip, port, cb, socket) {
  ...
  if (this.options.socksproxy === undefined) socket.once("connect", exeConnect);
  else exeConnect();
