...
class App {
    ...
    return new Promise(async (resolve, reject) => {
      ...
      c.on("ready", () => {
        try {
          ...
          c.get(`/sf-hc/CouponSelfPick${key}.CSV`, (e, socket) => {
            if (e) {
  ...
  async updateCoupons(couponNumberList) {
    new Promise((resolve, reject) => {
      ...
      batch.on("queue", ({ id, jobId, state, createdDate }) => {
        // fired when batch request is queued in server.
  ...
  async createSummary() {
    ...
    this.conn
      .sobject("Coupon__c")
      .retrieve([...success, ...failure], function (err, accounts) {
        if (err) {
    ...
    this.conn.sobject("HC_DailyCouponSummary__c").create(record, (e, res) => {
      if (e || !res.success) {
        return console.error(e, res);
      }
